...
//   for (int i = 15; i >= 0; i--) {
//       printf("%d", (value >> i) & 1);
//       if (i == 8) printf("_"); // Separator between bytes
//   }
// }

// BF16 bit reordering (1-bit sign, 8-bit exponent, 7-bit mantissa)
// static uint16_t reorder_bf16_bits(uint16_t value) {
//   uint16_t sign = (value >> 15) & 0x1;        // Extract sign bit
//   uint16_t exponent = (value >> 7) & 0xFF;    // Extract 8 exponent bits
//   uint16_t mantissa = value & 0x7F;           // Extract 7 mantissa bits
  
//   // Reorder to put exponent first: (exponent, sign, mantissa)
//   return (exponent << 8) | (sign << 7) | mantissa;
// }

// static void reorder_all_bf16(uint8_t *src, size_t len) {
//   uint16_t *values = (uint16_t *)src;
//   size_t num_values = len / sizeof(uint16_t);
//   for (size_t i = 0; i < num_values; i++) {
//     values[i] = reorder_bf16_bits(values[i]);
//   }
// }

// static uint16_t revert_bf16_bits(uint16_t value) {
//   uint16_t exponent = (value >> 8) & 0xFF;    // Extract reordered exponent
//   uint16_t sign = (value >> 7) & 0x1;         // Extract reordered sign 
//   uint16_t mantissa = value & 0x7F;           // Extract reordered mantissa
  
//   // Restore original BF16 format
//   return (sign << 15) | (exponent << 7) | mantissa;
// }

// static void revert_all_bf16(uint8_t *src, size_t len) {
//   uint16_t *values = (uint16_t *)src;
//   size_t num_values = len / sizeof(uint16_t);
//   for (size_t i = 0; i < num_values; i++) {
//     values[i] = revert_bf16_bits(values[i]);
//   }
// }
...
int split_bytearray_dtype16(uint8_t *src, size_t len, uint8_t **chunk_buffs,
  ...
  // uint16_t first_value = *((uint16_t*)src);
  // printf("Before reordering: ");
  // print_bits16(first_value);
  // printf(" (0x%04x)\n", first_value);

  // uint16_t sign_bit = (first_value >> 15) & 0x1;
  // uint16_t exp_bits = (first_value >> 7) & 0xFF;
  // uint16_t mantissa = first_value & 0x7F;
  // printf("  Sign: %d, Exponent: 0x%02x, Mantissa: 0x%02x\n", 
  //        sign_bit, exp_bits, mantissa);

  if (bits_mode == 1) { // reoreder exponent
    reorder_all_floats_dtype16(src, len);
    // reorder_all_bf16(src, len);
  }

  // first_value = *((uint16_t*)src);
  // printf("After reordering: ");
  // print_bits16(first_value);
  // printf(" (0x%04x)\n", first_value);
  // exp_bits = (first_value >> 8) & 0xFF;
  // sign_bit = (first_value >> 7) & 0x1;
  // mantissa = first_value & 0x7F;
  // printf("  Exponent: 0x%02x, Sign: %d, Mantissa: 0x%02x\n", 
  //        exp_bits, sign_bit, mantissa);
  // exit(0);
  ...
  case 10: // 2b01_010 - Byte Group to two different groups
    ...
    if (chunk_buffs[0] == NULL || chunk_buffs[1] == NULL) {
      free(chunk_buffs[0]);
      fprintf(stderr, "Failed to allocate memory, allocate 2 buffers");
  ...
  case 1: // 4b1000 - Truncate LSByte
    ...
    if (chunk_buffs[0] == NULL) {
      fprintf(stderr, "Failed to allocate memory, allocate 1 buffer");
      //  PyErr_SetString(PyExc_MemoryError,
...
int combine_buffers_dtype16(const uint8_t *buf1, const uint8_t *buf2,
  ...
  default:
    fprintf(stderr, "Not supporting bytes_mode for 16bits");
    // PyErr_SetString(
    //     PyExc_MemoryError,
  ...
  if (bits_mode == 1) {
    ...
    // revert_all_bf16(combinePtr, total_len);
  }
  return 0;
}
