...
uint8_t *prepare_return_buffer(
  ...
  *resBufSize += compChunksTypeLen + cumulativeChunksSizeLen;
  ...
  if (!resultBuf) {
    fprintf(stderr, "Failed to allocate memory for result buffer in split function");
    // PyErr_SetString(
    //     PyExc_MemoryError,
  ...
  while (1) {
    ...
    size_t curOrigChunkSize =
        (current_chunk == thread_data->numChunks - 1)
            ? (thread_data->data_len - offset) // Last chunk
    ...
    if (thread_data->numBuf == 2) {
      ...
      if (split_bytearray_dtype16(
              thread_data->data + offset, curOrigChunkSize,
    ...
    for (uint32_t b = 0; b < thread_data->numBuf; b++) {
      ...
      if (thread_data->buffers[current_chunk][b] != NULL) {
        ...
        if (thread_data->compChunksSize[b][current_chunk] != 0 &&
          ...
          // Compression beneficial - use original data
...
// data_len was data.len previously
// header_len was header.len previously
// out_buffer and out_buffer_size are new arguments
...
//   size_t numChunks = (data.len + origChunkSize - 1) / origChunkSize;
  size_t numChunks = (data_len + origChunkSize - 1) / origChunkSize;
  ...
  if (!buffers || !unCompChunksSize) {
    ...
    fprintf(stderr, "Failed to allocate buffers || unCompChunksSize");
  ...
  for (size_t c = 0; c < numChunks; c++) {
    ...
    if (!buffers[c] || !unCompChunksSize[c]) {
      fprintf(stderr, "Failed to allocate buffers[c] || unCompChunksSize[c]");
      goto compression_error;
    }
  }
  ...
  if (!compressedData || !compChunksType || !compChunksSize) {
    fprintf(stderr, "Failed to allocate compressedData || compChunksType || compChunksSize");
    goto compression_error;
  }
  ...
  for (uint32_t b = 0; b < numBuf; b++) {
    ...
    fprintf(stderr, "Failed to allocate compressedData[b] || compChunksType[b] || compChunksSize[b]");
      goto compression_error;
    }
  }
  ...
  for (uint32_t i = 0; i < threads; i++) {
    ...
    if (thread_result != NULL) {
      fprintf(stderr, "Thread processing failed");
      goto cleanup_threads;
    }
  }
...
//       compChunksType, totalCompressedSize, &resBufSize, threads);
  ...
  resultBuf = prepare_return_buffer(
    header_len, numBuf, numChunks, header, compressedData, compChunksSize,
    compChunksType, totalCompressedSize, &resBufSize, threads);
  
  *out_buffer = resultBuf;
  *out_buffer_size = resBufSize;
...
int zipnn_decompress(
  ...
  if (!compressed_data || !out_buffer || !out_buffer_size) {
  ...
  if (1) { // TBD when support auto byte_reorder
    if (numBuf == 2) {
      if (buffer_ratio_dtype16(bytes_mode, oneBufRatio) == -1) {
        fprintf(stderr, "Failed to calculate buffer ratio");
        *out_buffer = NULL;
        // *out_buffer_size = 0;
        return 0;
...
//   uint8_t *ptrChunksType = (uint8_t *)data.buf;
  uint8_t *ptrChunksType = compressed_data + 32;
  ...
  if (deCompressedDataPtr == NULL) {
    fprintf(stderr, "Failed to allocate deCompressedDataPtr");
  ...
  for (uint32_t b = 0; b < numBuf; b++) {
    ...
    //    PyErr_SetString(PyExc_MemoryError, "Failed to allocate deCompressedDataPtr[b]");
      fprintf(stderr, "Failed to allocate deCompressedDataPtr[b]");
      goto decompression_error;
    }
    for (size_t c = 0; c < numChunks; c++) {
    ...
    for (uint32_t b = 0; b < numBuf; b++) {
      if (compChunksType[b][c] == 0) { // no compression is needed
        ...
        if (compChunksType[b][c] == 1) { // open with Huffman compression
          ...
          fprintf(stderr, "Compress Type is not correct in Decompression function");
          *out_buffer = NULL;
          return 0;
        //   PyErr_SetString(
        //       PyExc_MemoryError,
  ...
  if (!resultBuf) {
        ...
        fprintf(stderr, "Failed to allocate resultBuf");
        goto decompression_error;
    return 0;
  }
  // eTime = clock();
...
int compress_uint16_data(
    ...
    size_t data_len_bytes = data_len_elements * sizeof(uint16_t);
    ...
    // Version info (0.5.2 from Python code)
    header[2] = 0;  // major
    header[3] = 5;  // minor
    header[4] = 2;  // tiny
    
    // Compression parameters
    header[5] = 10;  // byte_reorder - 0b01_010 for 16-bit data
    if (bits_mode == 0) {
      header[6] = 0; // bit_reorder - no bit reordering for uint16
    }
    else if (bits_mode == 1) {
      header[6] = 1;
    }
    else {
      fprintf(stderr, "Invalid bits_mode: %u\n", bits_mode);
      return 0;  // Invalid bits_mode
    }
    header[7] = 1;   // method - AUTO (from EnumMethod)
    header[8] = 0;   // input_format - BYTE (from EnumFormat)
    header[9] = 0;   // delta_compressed_type - none
    
    // No lossy compression
    header[10] = 0;  // lossy_compressed_type - none
    header[11] = 0;  // lossy_compressed_factor
    header[12] = 0;  // lossy_is_int
    
    // Chunk sizes
    header[13] = 0;                 // not streaming
    header[14] = 18;                // compression_chunk = 2^18 = 256KB
    
    // Set dtype for uint16
    header[15] = 4;                 // UINT16 dtype code

    // Original data length (in bytes)
    memcpy(&header[16], &data_len_bytes, 8);
    
    // Set default threads if 0
    if (threads == 0) {
        threads = 48;  // Default to 48 threads
    }
    
    // Parameters for compression
    uint32_t numBuf = 2;           // Always 2 for 16-bit data
    // uint32_t bits_mode = 0;        // No bit reordering for uint16
    uint32_t bytes_mode = 10;      // Matches byte_reorder for 16-bit. 8 = compress first buffer and drop the rest, 1 = compress second buffer and drop the rest, 10 = compress both
    uint32_t is_redata = 0;        // Not reordering data
    size_t origChunkSize = 256*1024; // 256KB chunks (2^18)
    float compThreshold = 0.95;    // Compression threshold
    uint32_t checkThAfterPercent = 10; // Check threshold after 10%
    
    // Cast uint16_t* to uint8_t* for the API
    uint8_t* data_bytes = (uint8_t*)data;
    
    // Call zipnn_compress
    return zipnn_compress(
        header, sizeof(header),
        data_bytes, data_len_bytes,
        numBuf,
...
int decompress_uint16_data(
  ...
  size_t origSize;
  memcpy(&origSize, &compressed_data[16], 8); // number of elements is stored in 8 bytes in the header

  uint32_t numBuf = 2;           // Same as in compress_uint16_data
  // uint32_t bits_mode = 0;        // Same as in compress_uint16_data
  uint32_t bytes_mode = 10;      // Same as in compress_uint16_data
  size_t origChunkSize = 256*1024; // Same as in compress_uint16_data
  // size_t origSize = num_elements * sizeof(uint16_t); // Original data size

  return zipnn_decompress(
    compressed_data, 
    numBuf, 
    bits_mode, 
    bytes_mode,
