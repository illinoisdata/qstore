...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Huffman compression analysis for 8-bit models")
...
def process_layer_huffman_8bit(layer_name):
    ...
    if f"{layer_name}.qweight" in st_8bit:
        ...
        huffman_codec = dahuffman.HuffmanCodec.from_data(np_8bit)
        compressed = huffman_codec.encode(np_8bit)
        
        original_bits = len(np_8bit) * 8
        compressed_bits = len(compressed) * 8
        avg_bits = compressed_bits / len(np_8bit)
...
huffman_8bit_results = {}

print(f"Processing {len(common_layers)} layers using {args.num_workers} threads...")

with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
    # Submit all tasks
    future_to_layer = {executor.submit(process_layer_huffman_8bit, layer_name): layer_name 
    ...
    for future in future_to_layer:
        ...
        if result is not None:
            huffman_8bit_results[layer_name] = result
        completed += 1
        if completed % 10 == 0:
            print(f"Completed {completed}/{len(common_layers)} layers")

print(f"Finished processing all {len(huffman_8bit_results)} layers with valid 8-bit weights")

# Compute aggregate statistics
total_orig_size_8bit = 0
total_compressed_size_8bit = 0
total_num_values_8bit = 0

for layer_name in common_layers:
    total_orig_size_8bit += huffman_8bit_results[layer_name]['original_size']
    total_compressed_size_8bit += huffman_8bit_results[layer_name]['compressed_size']
    total_num_values_8bit += huffman_8bit_results[layer_name]['num_values']

print(f"Total original size: {total_orig_size_8bit}")
print(f"Total compressed size: {total_compressed_size_8bit}")
print(f"Total num values: {total_num_values_8bit}")
total_compressed_bits = total_compressed_size_8bit * 8 * 1024 * 1024 * 1024
print(f"Total avg bits per value: {total_compressed_bits / total_num_values_8bit}")
print(f"Compression ratio: {total_orig_size_8bit / total_compressed_size_8bit:.2f}x")
