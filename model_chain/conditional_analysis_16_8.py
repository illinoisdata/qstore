...
def setup_logging():
    ...
    log_dir = Path("conditional_analysis_16_8_logs")
    log_dir.mkdir(exist_ok=True)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = log_dir / f"conditional_analysis_16_8_{timestamp}.log"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Conditional analysis 16|8 QStore encoding")
...
def load_models(model_dir, model_name):
    ...
    # Load 16-bit model
    print("Loading 16-bit FP16 model...")
    ...
    return st_16bit, st_8bit

# Parse arguments and load models
args = parse_args()
st_16bit, st_8bit = load_models(args.model_dir, args.model_name)

def get_common_weight_layers():
    """Get layer names that exist in both 16-bit and 8-bit models"""
    layers_16bit = set()
    layers_8bit = set()
    ...
    # Find common layers
    common_layers = list(layers_16bit & layers_8bit)
...
def process_single_layer(args):
    ...
    try:
        ...
        result = vectorized_conditional_huffman_16_8_no_rounding(layer_name)
        end_time = time.time()
...
# Conditional Huffman encoding for 16-bit weights given 8-bit weights
def vectorized_conditional_huffman_16_8_no_rounding(layer_name):
    """
    Vectorized implementation for conditional encoding: 16-bit weights | 8-bit weights
    Uses exact scale values for precise grouping
    """
    print(f"Vectorized conditional 16|8 analysis (no rounding) for {layer_name}...")
    
    # Get required tensors
    orig_weight = st_16bit[f"{layer_name}.weight"].T  # Shape: [input_dim, output_dim]
    scales = st_8bit[f"{layer_name}.scales"]  # Shape: [num_groups, output_dim]
    g_idx = st_8bit[f"{layer_name}.g_idx"]    # Shape: [input_dim]
    ...
    # Step 5: Extract all 8-bit quantized values
    # For 8-bit: each weight takes 1 byte, so packing factor is 4 (32-bit integers / 8 bits per weight)
    packed_indices = input_flat // 4  # 4 weights per 32-bit integer
    bit_positions = input_flat % 4     # Position within the packed integer
    packed_vals = qweight_8bit[packed_indices, output_flat]
    bit_shifts = bit_positions * 8     # 8 bits per weight
    quantized_8bit_flat = (packed_vals >> bit_shifts) & 0xFF  # Extract 8 bits [input_dim * output_dim]
    ...
    for i in range(0, len(unique_scales), batch_size):
        ...
        for scale_val in batch_scales:
            ...
            scale_quantized = quantized_8bit_cpu[scale_mask]
...
def main():
    ...
    logging.info("=== Starting Conditional Analysis 16|8 with Parallelization ===")
    ...
    summary = {
        ...
        'analysis_type': '16bit_given_8bit'
    }
