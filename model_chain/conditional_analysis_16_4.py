...
def setup_logging():
    ...
    log_dir = Path("conditional_analysis_logs")
    log_dir.mkdir(exist_ok=True)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = log_dir / f"conditional_analysis_{timestamp}.log"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Conditional analysis 16|4 QStore encoding")
...
model_4bit = AutoModelForCausalLM.from_pretrained(
    os.path.join(args.model_dir, f"{args.model_name}-gptq-4bit"),
    device_map="auto"
)

st_4bit = model_4bit.state_dict()

print(st_4bit["model.layers.0.self_attn.k_proj.qweight"].shape)
print(st_4bit["model.layers.0.self_attn.k_proj.qzeros"].shape)
print(st_4bit["model.layers.0.self_attn.k_proj.scales"].shape)
print(st_4bit["model.layers.0.self_attn.k_proj.g_idx"].shape)

model_8bit = AutoModelForCausalLM.from_pretrained(
    os.path.join(args.model_dir, f"{args.model_name}-gptq-8bit"),
    device_map="auto"
)
...
model_fp16 = AutoModelForCausalLM.from_pretrained(
    os.path.join(args.model_dir, f"{args.model_name}-fp16"),
    device_map="auto",
)

st_16bit = model_fp16.state_dict()
print(st_16bit['model.layers.0.self_attn.k_proj.weight'].shape)
...
def process_single_layer(args):
    ...
    try:
        ...
        result = vectorized_conditional_huffman_16_4_no_rounding(layer_name)
        end_time = time.time()
...
# Fixed implementation without rounding scale values
def vectorized_conditional_huffman_16_4_no_rounding(layer_name):
    """
    Fixed vectorized implementation without rounding scale values
    Uses exact scale values for precise grouping
    """
    print(f"Vectorized conditional 16|4 analysis (no rounding) for {layer_name}...")
    
    # Get required tensors
    orig_weight = st_16bit[f"{layer_name}.weight"].T  # Shape: [input_dim, output_dim]
    scales = st_4bit[f"{layer_name}.scales"]  # Shape: [num_groups, output_dim]
    g_idx = st_4bit[f"{layer_name}.g_idx"]    # Shape: [input_dim]
    qweight_4bit = st_4bit[f"{layer_name}.qweight"]  # Shape: [packed_input_dim, output_dim]
    
    input_dim_size, output_dim_size = orig_weight.shape
    device = orig_weight.device
    
    print(f"  Processing {input_dim_size} x {output_dim_size} weight matrix...")
    print(f"  Groups: {scales.shape[0]}, Group size: {input_dim_size // scales.shape[0]}")
    ...
    # Step 2: Get group IDs for all positions
    g_idx = g_idx.to(device)
    scales = scales.to(device)
    qweight_4bit = qweight_4bit.to(device)
    
    group_ids_flat = g_idx[input_flat]  # [input_dim * output_dim]
    
    # Step 3: Extract all scale values
    scale_vals_flat = scales[group_ids_flat, output_flat]  # [input_dim * output_dim]
    
    # Step 4: Extract all 16-bit weights
    weights_16bit_flat = orig_weight[input_flat, output_flat]  # [input_dim * output_dim]
    
    # Step 5: Extract all 4-bit quantized values
    packed_indices = input_flat // 8
    bit_positions = input_flat % 8
    packed_vals = qweight_4bit[packed_indices, output_flat]
    bit_shifts = bit_positions * 4
    quantized_4bit_flat = (packed_vals >> bit_shifts) & 0xF  # [input_dim * output_dim]
    
    # Convert to CPU with appropriate data types (no need for float64)
    scale_vals_cpu = scale_vals_flat.cpu().numpy().astype(np.float32)  # float32 sufficient for exact comparison
    quantized_4bit_cpu = quantized_4bit_flat.cpu().numpy().astype(np.uint8)  # uint8 sufficient for 4-bit values (0-15)
    weights_16bit_cpu = weights_16bit_flat.cpu().numpy().astype(np.float32)  # float32 sufficient for Huffman
    
    # Step 6: Group by exact scale values first, then by quantized values
    print(f"  Grouping by exact scale values...")
    
    # Use exact scale values without rounding
    unique_scales = np.unique(scale_vals_cpu)
    ...
    for i in range(0, len(unique_scales), batch_size):
        ...
        for scale_val in batch_scales:
            ...
            scale_mask = scale_vals_cpu == scale_val
            scale_weights = weights_16bit_cpu[scale_mask]
            scale_quantized = quantized_4bit_cpu[scale_mask]
            
            # Group by quantized values within this scale
            unique_quantized = np.unique(scale_quantized)
            
            for q_val in unique_quantized:
                # Get all weights for this (scale_value, quantized_value) combination
                q_mask = scale_quantized == q_val
                subgroup_weights = scale_weights[q_mask]
                
                if len(subgroup_weights) > 1:
                    try:
                        # Apply Huffman coding
                        huffman_codec = dahuffman.HuffmanCodec.from_data(subgroup_weights)
                        compressed = huffman_codec.encode(subgroup_weights)
                        subgroup_bits = len(compressed) * 8
                    except Exception as e:
                        # Fallback to no compression if Huffman fails
                        print(f"    Huffman failed for scale {scale_val:.12f}, q_val {q_val}: {e}")
                        subgroup_bits = len(subgroup_weights) * 16
                else:
                    # Single value, no compression needed
                    subgroup_bits = 16
                
                total_conditional_bits += subgroup_bits
                total_weights += len(subgroup_weights)
    
    print(f"  Completed! Total weights: {total_weights}, Total bits: {total_conditional_bits}")
    
    # Calculate metrics
    original_bits = total_weights * 16
...
def main():
    ...
    logging.info("=== Starting Conditional Analysis with Parallelization ===")
