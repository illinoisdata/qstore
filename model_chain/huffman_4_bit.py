...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Huffman compression analysis for 4-bit models")
...
def extract_all_4bit_weights_gpu(qweight):
    ...
    # Flatten and return as list (convert to CPU only at the end)
    return extracted.flatten().cpu().numpy().astype(np.uint8).tolist()

def process_layer_huffman_4bit(layer_name):
    ...
    if f"{layer_name}.qweight" in st_4bit:
        ...
        huffman_codec = dahuffman.HuffmanCodec.from_data(np_4bit)
        compressed = huffman_codec.encode(np_4bit)
        
        original_bits = len(np_4bit) * 4
        compressed_bits = len(compressed) * 8
        avg_bits = compressed_bits / len(np_4bit)
...
huffman_4bit_results = {}

print(f"Processing {len(common_layers)} layers using {args.num_workers} threads...")

with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
    # Submit all tasks
    future_to_layer = {executor.submit(process_layer_huffman_4bit, layer_name): layer_name 
    ...
    for future in future_to_layer:
        ...
        if result is not None:
            huffman_4bit_results[layer_name] = result
        completed += 1
        if completed % 10 == 0:
            print(f"Completed {completed}/{len(common_layers)} layers")

print(f"Finished processing all {len(huffman_4bit_results)} layers with valid 4-bit weights")
...
for layer_name in common_layers:
    total_orig_size += huffman_4bit_results[layer_name]['original_size']
    total_compressed_size += huffman_4bit_results[layer_name]['compressed_size']
    total_num_values += huffman_4bit_results[layer_name]['num_values']

print(f"Total original size: {total_orig_size}")
print(f"Total compressed size: {total_compressed_size}")
print(f"Total num values: {total_num_values}")
total_compressed_bits = total_compressed_size * 8 * 1024 * 1024 * 1024
print(f"Total avg bits per value: {total_compressed_bits / total_num_values}")
